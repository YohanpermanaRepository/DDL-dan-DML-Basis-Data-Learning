CREATE DATABASE peminjaman_buku_3;

use peminjaman_buku_3;
...
## Soal nomor 1
DELIMITER //
CREATE PROCEDURE ambildata(IN p_IdAnggota varchar(10))
BEGIN 
	SELECT * FROM anggota WHERE IdAnggota = p_IdAnggota;
END //
DELIMITER ;

call ambildata('AGT001');


## Soal nomor 2
DELIMITER //
CREATE PROCEDURE ambilpetugas(IN p_id_petugas varchar (10))
BEGIN
  SELECT * FROM petugas WHERE IdPetugas = p_id_petugas;
END //
DELIMITER ;

call ambilpetugas('PT01');

## Soal no 3


DELIMITER //
CREATE PROCEDURE ambilbuku (IN p_Judul_Buku varchar(30), IN p_Penerbit_Buku varchar(30))
BEGIN 
	SELECT * FROM buku WHERE Judul_Buku LIKE CONCAT('%', p_Judul_Buku, '%') OR Penerbit_Buku LIKE CONCAT('%', p_Penerbit_Buku, '%');
END //
DELIMITER ;

call ambilbuku('ashaoshaohsa','Secker and Warburg');
 

## Soal no 4

DELIMITER //
CREATE PROCEDURE ambilbuku2 (IN p_Judul_Buku varchar(30), IN p_Penerbit_Buku varchar(30), IN p_Pengarang_Buku varchar (30))
BEGIN 
	SELECT * FROM buku WHERE Judul_Buku LIKE CONCAT('%', p_Judul_Buku, '%')
    OR Penerbit_Buku LIKE CONCAT('%', p_Penerbit_Buku, '%')
    OR Pengarang_Buku LIKE CONCAT('%', p_Pengarang_Buku, '%');
END //
DELIMITER ;
call ambilbuku2('ashaoshaohsa','asasasasasa','Paulo Coelho');
...
('PT06', 'angga', 'usecase123', 'Angga Dirga');
...
call ambildataanggota ('AGT001',@IdAnggota_out, @NamaAnggota_out, @Angkatan_out);
SELECT @IdAnggota_out, @NamaAnggota_out, @Angkatan_out;
